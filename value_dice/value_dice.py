...
class ValueDICE(object):
  ...
  def __init__(self, state_dim, action_dim, log_interval, nu_lr, actor_lr,
               alpha_init, hidden_size, discrete=False):
    ...
    self.actor = twin_sac.Actor(state_dim, action_dim, discrete=self.discrete)
  ...
  def update(self,
    ...
    with tf.GradientTape(
      ...
      if not self.discrete:
        ...
        mat = tf.one_hot(tf.cast(expert_actions, tf.int32), depth=self.action_dim, axis=-1)
      ...
      linear_loss = (
          linear_loss_expert * (1 - replay_regularization) + 0)
      ...
      nu_inter = alpha * expert_inputs + (1 - alpha) * tf.stop_gradient(tf.random.shuffle(expert_next_inputs))
