...
flags.DEFINE_float('replay_regularization', 0.0, 'Amount of replay mixing.')
...
flags.DEFINE_integer('max_timesteps', int(1e3) + 1, 'Max timesteps to train.')
...
def main(_):
  ...
  (expert_states, expert_actions, expert_next_states,
   expert_dones) = data_utils.load_expert_data(filename, FLAGS.num_trajectories)
  ...
  if isinstance(env.action_space, gym.spaces.Box):
    ...
    spec = (
        tensor_spec.TensorSpec([env.observation_space.shape[0]], tf.float32,
                              'observation'),
        tensor_spec.TensorSpec([1], tf.float32, 'action'),
        tensor_spec.TensorSpec([env.observation_space.shape[0]], tf.float32,
                              'next_observation'),
        tensor_spec.TensorSpec([1], tf.float32, 'reward'),
        tensor_spec.TensorSpec([1], tf.float32, 'mask'),
  ...
  with tqdm(total=FLAGS.max_timesteps, desc='') as pbar:
    while total_timesteps < FLAGS.max_timesteps:
      ...
      if total_timesteps % FLAGS.eval_interval == 0:
        ...
        avg_ret = np.mean(average_returns)
        if total_timesteps == 0:
          if os.path.isfile("../pillbox/learners/{0}/valuedice_rewards_{1}.npz".format(FLAGS.env_name, FLAGS.seed)):
            pass
          else:
            np.savez("../pillbox/learners/{0}/valuedice_rewards_{1}".format(FLAGS.env_name, FLAGS.seed),
            means=np.array([avg_ret]))
        if total_timesteps == FLAGS.max_timesteps - 1:
          means = np.load("../pillbox/learners/{0}/valuedice_rewards_{1}.npz".format(FLAGS.env_name, FLAGS.seed))["means"]
          means = np.concatenate([means, [avg_ret]])
          np.savez("../pillbox/learners/{0}/valuedice_rewards_{1}".format(FLAGS.env_name, FLAGS.seed),
          means=means)
