...
def load_expert_data(filename, max_trajs):
  ...
  """
  envs = ['Pendulum-v0', 'LunarLander-v2', 'HalfCheetahBulletEnv-v0', 'Walker2DBulletEnv-v0',
          'HopperBulletEnv-v0', 'AntBulletEnv-v0']
  for env in envs:
    if env in filename:
      demos = np.load("../pillbox/experts/{0}/demos.npz".format(env), allow_pickle=True)
  ...
  print('TOTAL DEMOS', len(expert_states))
  expert_states = np.array(expert_states)
  expert_actions = np.array(expert_actions)
  expert_next_states = np.array(expert_next_states)
  expert_dones = np.array(expert_dones)
